...
#Author: Max Gordon
#version: 1.9.1
#Modified by: AVH - changes format to n/N (%)
#Description: Creating of description statistics
#Input: NA
#Output:NA
#________________________________________#
#' Describe the mean
                             ...
                             ...)
{
    ...
    if (number_first)
        ret <- matrix(sprintf("%s / %s (%s)", values, denom, percentage_str), ncol=1)
    else
        ret <- matrix(sprintf("%s (%s / %s)", percentage_str, values,denom), ncol=1)
